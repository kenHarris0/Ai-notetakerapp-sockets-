...
useEffect(()=>{
  ...
  return (
    <div className='w-[80%] h-15 m-auto bg-black text-white flex items-center justify-between 
fixed top-0 left-1/2 -translate-x-1/2 gap-10 p-3 z-50'>
        ...
        <div className='w-[40%] flex items-center justify-between  gap-7'>
            ...
            <div className='flex items-center jsutify-center gap-10'>
                <p className='cursor-pointer' onClick={()=>navv('/notepg')}>Notes</p>
        ...
        <div className=' w-[40%] flex items-center justify-end pr-10 '>
            {userdata ? <h1 className='text-[30px] flex items-center justify-center  w-10 h-10 border-3 bg-gray-600 rounded-full cursor-pointer' onClick={()=>setshowdropdown(prev=>!prev)}>{userdata?.name?.[0]?.toUpperCase()}</h1> : <p className='cursor-pointer' onClick={()=>navv('/login')}>Login</p>}
             {showdropdown && (
  <div className="absolute top-14 right-0 w-32 bg-gray-800 rounded-md shadow-lg border border-gray-700">
    <button
      onClick={logout}
      className="w-full px-4 py-2 text-left text-sm hover:bg-gray-700 cursor-pointer"
    >
      Logout
    </button>
  </div>
